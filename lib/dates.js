...
// 5 ngay hoc (Thu 2 -> Thu 6) cua TUAN CHUA ngay `mondayIso` (phai la 1 Thu Hai).
// Dung cho Sao do khi TPT mo lai 1 tuan cu de nhap bu - khac getWeekdays() o cho
// KHONG gioi han vao tuan hien tai, va isFuture/isToday van tinh theo hom nay that.
export function weekdaysForMonday(mondayIso) {
  const todayIso = vnTodayIso();
  const days = [];
  for (let i = 0; i < 5; i++) {
