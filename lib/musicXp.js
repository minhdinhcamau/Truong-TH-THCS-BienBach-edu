...
export async function finishMusicLessonAttempt({
  ...
  // Mở khoá cấp độ kế tiếp (Chậm -> Vừa -> Nhanh) nếu đạt.
  if (completed) {
    const nextLevel = level === 'slow' ? 'medium' : level === 'medium' ? 'fast' : null;
    if (nextLevel) {
      await supabase.from('music_lesson_progress').upsert(
      ...
      // Đã qua cả 3 cấp độ của bài này -> mở bài kế tiếp (cấp Chậm)
      await supabase.from('music_lesson_progress').upsert(
        { student_id: studentId, lesson_id: nextLessonId, level: 'slow', is_unlocked: true },
        { onConflict: 'student_id,lesson_id,level' }
      );
    }
  }
